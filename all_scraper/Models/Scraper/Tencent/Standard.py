...
from Models.Processor.Tencent.Base import Model_Processor_Tencent_Base
from Models.Processor.Tencent.Blog import Model_Processor_Tencent_Blog
from pyvirtualdisplay import Display

from Downloader.Selenium import Downloader_Selenium
from requests import RequestException

# from Models.Processor.Tencent.Edu import Model_Processor_Tencent_Edu


class Model_Scraper_Tencent_Standard():
    def __init__(self):
        pass
    ...
    def process_blog(self, url):
        with Display(backend="xvfb", size=(1440, 900)):
            ...
            try:
                self.processor = Model_Processor_Tencent_Blog()
                count = 0
                data = []
                while True:
                    time.sleep(1)
    ...
    def process_edu(self, url):
        with Display(backend="xvfb", size=(1440, 900)):
            ...
            # 抓取新浪博客时需要进行相应操作，点击下一页
            try:
                self.processor = Model_Processor_Tencent_Base()
                count = 0
                data = []
                while True:
                    time.sleep(1)
