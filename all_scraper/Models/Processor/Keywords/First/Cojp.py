...
class Model_Processor_Keywords_First_Cojp(Model_Processor):
    def __init__(self):
        pass
    ...
    def process(self, html, page_id=1):
        ...
        try:
            ...
            if (listDoms):
                ...
                for itemDom in listDoms:
                    ...
                    try:
                        ...
                        if (price):
                            if (Model_Processor().formatNumber(price[0], "co.jp").isdigit()):
                                item['price'] = Model_Processor().formatNumber(price[0], "co.jp")
                        else:
                    ...
                    try:
                        ...
                        if (list_price):
                            if (Model_Processor().formatNumber(list_price[0], "co.jp").isdigit()):
                                item['list_price'] = Model_Processor().formatNumber(list_price[0], "co.jp")
                    except:
                    ...
                    try:
                        ...
                        if (rating):
                            item['rating'] = Model_Processor().formatRating(rating[0], "co.jp")
                    except:
                        print ("rating error")
                    ...
                    try:
                        ...
                        if (review_count):
                            count = Model_Processor().formatNumber(review_count[0], "co.jp")
                            if (count.isdigit()):
                                item['review_count'] = count
                        else:
                            review_count = itemDom.xpath("div//*[contains(@class, 'a-row a-spacing-none')]/a/text()")
                            if (review_count):
                                for i in review_count:
                                    if (i.replace(",", "").strip().isdigit()):
                                        count = Model_Processor().formatNumber(i.replace(",", "").strip(), "co.jp")
                                        if (count.isdigit()):
                                            item['review_count'] = count
                    except:
                            ...
                            for itemDom in rightListDom:
                                ...
                                if (linkDoms):
                                    ...
                                    if not item['asin']:
                                        ...
                                        if (review_countDom):
                                            item['review_count'] = Model_Processor().formatNumber(review_countDom[0], "co.jp")
                                ...
                                try:
                                    ...
                                    if (priceDom):
                                        if (Model_Processor().formatNumber(priceDom[0], "co.jp").isdigit()):
                                            item['price'] = Model_Processor().formatNumber(priceDom[0], "co.jp")
                                except:
                                ...
                                try:
                                    ...
                                    if (list_priceDom):
                                        if (Model_Processor().formatNumber(list_priceDom[0], "co.jp").isdigit()):
                                            item['list_price'] = Model_Processor().formatNumber(list_priceDom[0], "co.jp")
                                except:
                                ...
                                try:
                                    ...
                                    if (ratingDom):
                                        item['rating'] = Model_Processor().formatRating(ratingDom[0], "co.jp")
                                except:
            ...
            if (totalDom):
                # 总数 >1
                # print (totalDom[0])
                total = Model_Processor().formatNumber(totalDom[0].split(" ")[1].replace("件中", ""), "co.jp")
                if (total.isdigit()):
                    total = {"total": total}
                    data.append(total)
                else:
                    # 总数 ==1
                    total = Model_Processor().formatNumber(totalDom[0].replace("件の結果", ""), "co.jp")
                    if (total.isdigit()):
                        total = {"total": total}
                        data.append(total)
