...
#############################################################
'''
class Model_Processor_Product_Review_Ca(Model_Processor):
        ...
        if (listDoms):
            for listDom in listDoms:
                ...
                try:
                    ...
                    if (ratingDom):
                        item['rating'] = self.formatRating(ratingDom[0], "ca")
                        # print (item['rating'])
                except Exception as err:
                    print (err)
                # title
                ...
                try:
                    if (item['author_id']):
                        item['author_url'] = "http://www.amazon.ca/gp/pdp/profile/"+item['author_id']
                ...
                try:
                    ...
                    if (review_dateDom):
                        item['date'] = self.formatReviewDate(review_dateDom[0], "ca")
                        # print (item['date'])
                except Exception as err:
                    print (err)
                # verified
    ...
    def getSummary(self, html):
        ...
        try:
            ...
            if (totalReviewCountDom):
                data['count'] = self.formatNumber(totalReviewCountDom[0], "ca")
                data['page_count'] = int(self.formatNumber(totalReviewCountDom[0], "ca"))/10 + 1
        except:
            pass
        # rating
        try:
            ...
            if (ratingDom):
                data['rating'] = self.formatRating(ratingDom[0], "ca")
                # print (data['rating'])
        except:
            pass
        if (len(data) > 0):
