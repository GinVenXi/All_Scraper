...
#############################################################
'''
class Model_Processor_Product_Review_Es(Model_Processor):
        ...
        if (listDoms):
            for listDom in listDoms:
                ...
                try:
                    ...
                    if (ratingDom):
                        item['rating'] = self.formatRating(ratingDom[0], "es")
                        # print (item['rating'])
                except Exception as err:
                    print (err)
                # title
                ...
                try:
                    if (item['author_id']):
                        item['author_url'] = "http://www.amazon.es/gp/pdp/profile/"+item['author_id']
                ...
                try:
                    ...
                    if (review_dateDom):
                        item['date'] = self.formatReviewDate(review_dateDom[0], "es")
                        # print (item['date'])
                except Exception as err:
                    print (err)
                # verified
                try:
                    ...
                    if (verifiedDom):
                        if (verifiedDom[0] == "Compra verificada"):
                            item['verified'] = 1
                            # print (item['verified'])
                except Exception as err:
                    print (err)
                ...
                try:
                    ...
                    if (helpful_yesDom):
                        helpful_yes_count = self.formatNumber(helpful_yesDom[0].strip().split(" ")[1], "es")
                        if (helpful_yes_count.isdigit()):
                            item['helpful_yes'] = self.formatNumber(helpful_yesDom[0].strip().split(" ")[1], "es")
    ...
    def getSummary(self, html):
        ...
        try:
            ...
            if (totalReviewCountDom):
                data['count'] = self.formatNumber(totalReviewCountDom[0], "es")
                data['page_count'] = int(self.formatNumber(totalReviewCountDom[0], "es"))/10 + 1
        except:
            pass
        # rating
        try:
            ...
            if (ratingDom):
                data['rating'] = self.formatRating(ratingDom[0], "es")
                # print (data['rating'])
        except:
            pass
        if (len(data) > 0):
