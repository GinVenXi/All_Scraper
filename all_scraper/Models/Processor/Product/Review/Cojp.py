...
#############################################################
'''
class Model_Processor_Product_Review_Cojp(Model_Processor):
        ...
        if (listDoms):
            for listDom in listDoms:
                ...
                try:
                    ...
                    if (ratingDom):
                        item['rating'] = self.formatRating(ratingDom[0], "co.jp")
                        # print (item['rating'])
                except Exception as err:
                    print (err)
                # title
                ...
                try:
                    if (item['author_id']):
                        item['author_url'] = "http://www.amazon.co.jp/gp/pdp/profile/"+item['author_id']
                ...
                try:
                    ...
                    if (review_dateDom):
                        item['date'] = self.formatReviewDate(review_dateDom[0], "co.jp")
                        # print (item['date'])
                except Exception as err:
                    print (err)
                # verified
                ...
                try:
                    ...
                    if (helpful_yesDom):
                        helpful_yes_count = self.formatNumber(helpful_yesDom[0].strip().split("人の")[0], "co.uk")
                        if (helpful_yes_count.isdigit()):
                            item['helpful_yes'] = self.formatNumber(helpful_yesDom[0].strip().split("人の")[0], "co.uk")
    ...
    def getSummary(self, html):
        ...
        try:
            ...
            if (totalReviewCountDom):
                data['count'] = self.formatNumber(totalReviewCountDom[0], "co.jp")
                data['page_count'] = int(self.formatNumber(totalReviewCountDom[0], "co.jp"))/10 + 1
        except:
            pass
        # rating
        try:
            ...
            if (ratingDom):
                data['rating'] = self.formatRating(ratingDom[0], "co.jp")
                # print (data['rating'])
        except:
            pass
        if (len(data) > 0):
