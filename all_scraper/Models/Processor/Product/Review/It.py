...
#############################################################
'''
class Model_Processor_Product_Review_It(Model_Processor):
        ...
        if (listDoms):
            for listDom in listDoms:
                ...
                try:
                    ...
                    if (ratingDom):
                        item['rating'] = self.formatRating(ratingDom[0], "it")
                        # print (item['rating'])
                except Exception as err:
                    print (err)
                # title
                ...
                try:
                    if (item['author_id']):
                        item['author_url'] = "http://www.amazon.it/gp/pdp/profile/"+item['author_id']
                ...
                try:
                    ...
                    if (review_dateDom):
                        item['date'] = self.formatReviewDate(review_dateDom[0], "it")
                        # print (item['date'])
                except Exception as err:
                    print (err)
                # verified
                try:
                    ...
                    if (verifiedDom):
                        if (verifiedDom[0] == "Acquisto verificato"):
                            item['verified'] = 1
                            # print (item['verified'])
                except Exception as err:
                    print (err)
                ...
                try:
                    ...
                    if (helpful_yesDom):
                        helpful_yes_count = self.formatNumber(helpful_yesDom[0].strip().split(" ")[0], "it")
                        if (helpful_yes_count.isdigit()):
                            item['helpful_yes'] = self.formatNumber(helpful_yesDom[0].strip().split(" ")[0], "it")
                            # print (item['helpful_yes'])
                        elif (helpful_yes_count == "Una"):
                            item['helpful_yes'] = "1"
                            # print (item['helpful_yes'])
                except Exception as err:
                    print (err)
    ...
    def getSummary(self, html):
        ...
        try:
            ...
            if (totalReviewCountDom):
                data['count'] = self.formatNumber(totalReviewCountDom[0], "it")
                data['page_count'] = int(self.formatNumber(totalReviewCountDom[0], "it"))/10 + 1
        except:
            pass
        # rating
        try:
            ...
            if (ratingDom):
                data['rating'] = self.formatRating(ratingDom[0], "it")
                # print (data['rating'])
        except:
            pass
        if (len(data) > 0):
