...
class Model_Processor_Product_Base_Ca(Model_Processor):
    ...
    def process(self, html):
        ...
        try:
            ...
            if(price):
                if (price[0].strip() != '' and self.formatNumber(price[0].strip(), "ca").isdigit()):
            ...
            try:
                ...
                if (price and self.formatNumber(price[0].strip(), "ca").isdigit()):
                    data['price'] = self.formatNumber(price[0].strip(), "ca")
                    # print (price[0])
                else:
                            ...
                            if (price and self.formatNumber(price[0].replace("\n", "").strip(), "ca").isdigit()):
                                ...
                                if (price):
                                    ...
                                    if (price):
                                        ...
                                        price = tree.xpath("//*[@id='buybox']//*[contains(@class, 'kindle-price')]//td[contains(@class, 'a-color-price')]/text()")
                                        if (price):
                                            if (price[0].strip() != '' and self.formatNumber(price[0].strip(), "ca").isdigit()):
                                                data['price'] = self.formatNumber(price[0].strip(), "ca")
                                        else:
        ...
        try:
            if(listprice):
                if (listprice[0].strip() != ''):
                    data['list_price'] = self.formatNumber(listprice[0].strip(), "ca")
                ...
                if (listprice):
                    ...
                    listprice = tree.xpath("//*[@id='buybox']//span[contains(@class, 'a-text-strike')]/text()")
                    if (listprice):
                        data['list_price'] = self.formatNumber(listprice[0].replace("\n", "").strip(), "ca")
                    else:
                        print ("no list_price")
                ...
                if (shipping1 or shipping2):
                    ...
                    if (shipping):
                        ...
                        if(shipping):
                            ...
                            if (shipping):
                                shipping = shipping[0].replace("›", "").replace("&", "").strip()
                                if (shipping != ""):
                                    data['shipping'] = shipping
                            else:
                                data['shipping'] = ''
        ...
        try:
            if(seller_link):
                for sel in seller_link:
                    if(sel.text.strip() == "Ships from and sold by Amazon.ca."):
                        data['seller_name'] = 'Amazon.ca'
            ...
            if(first_available_date):
                ...
                for first_available in first_available_date:
                    for first in first_available:
                        try:
                            if(first.text != None):
                                if(first.text.strip() == "Date first available at Amazon.ca:"):
                                    first_cnt = 1
                                    continue
                                if(first_cnt == 1):
                                    data['first_available_date'] = self.formatDate(first.text.strip(), "ca")
                                    break
                            else:
                                break
                        except Exception as err:
                ...
                if (first_available_date):
                    for i in first_available_date:
                        if (i[0].text.strip() != ''):
                            if (i[0].text.strip() == "Date first available at Amazon.ca"):
                                data['first_available_date'] = self.formatDate(i[1].text.strip(), "ca")
                else:
                    ...
                    if (first_available_date):
                        for i in first_available_date:
                            try:
                                try:
                                    if (i.xpath("strong/text()")[0].strip() == "Date first available at Amazon.ca:"):
                                    ...
                                    pass
                                try:
                                    if (i.xpath("b/text()")[0].strip() == "Date first available at Amazon.ca:"):
                                        for j in i.xpath("text()"):
                                            if (j.replace("\n", "").strip() != ""):
                                                # print(j.replace("\n", "").strip())
                                                data['first_available_date'] = self.formatDate(j.replace("\n", "").strip(), "ca")
                                ...
                                try:
                                    ...
                                    pass
                                try:
                                    if (i.xpath("strong/text()")[0].strip() == "Audio CD"):
                                        for j in i.xpath("text()"):
                                            if (j.replace("\n", "").strip() != ""):
                                                # print(j.replace("\n", "").strip())
                                                data['first_available_date'] = self.formatDate(j.replace("\n", "").strip(), "ca")
                                    ...
                                    pass
                                try:
                                    if (i.xpath("b/text()")[0].strip() == "Audio CD"):
                                    ...
                                    pass
                                try:
                                    if (i.xpath("strong/text()")[0].strip() == "Release Date:"):
                                        for j in i.xpath("text()"):
                                            if (j.replace("\n", "").strip() != ""):
                                                # print(j.replace("\n", "").strip())
                                                data['first_available_date'] = self.formatDate(j.replace("\n", "").strip(), "ca")
                                    ...
                                    pass
                                try:
                                    if (i.xpath("b/text()")[0].strip() == "Release Date:"):
                                        for j in i.xpath("text()"):
                                            if (j.replace("\n", "").strip() != ""):
                                                # print(j.replace("\n", "").strip())
                                                data['first_available_date'] = self.formatDate(j.replace("\n", "").strip(), "ca")
                        ...
                        if (first_available_date):
                            try:
                                for i in first_available_date:
                                    ...
                                    if (i.xpath("b/text()")[0].strip() == "Publisher:"):
                                        if ("(" in i.xpath("text()")[0].strip()):
                                            # print (i.xpath("text/()")[0].strip().split("(")[1].strip())
                                            data['first_available_date'] = self.formatDate(i.xpath("text()")[0].strip().split("(")[1].strip(), "ca")
                                            break
                                        else:
                                            data['first_available_date'] = self.formatDate(i.xpath("text()")[0].strip(), "ca")
                                            break
                                    elif (i.xpath("b/text()")[0].strip() == "Date first available at Amazon.ca:"):
                                        if ("(" in i.xpath("text()")[0].strip()):
                                            # print (i.xpath("text/()")[0].strip().split("(")[1].strip())
                                            data['first_available_date'] = self.formatDate(
                                                i.xpath("text()")[0].strip().split("(")[1].strip(), "ca")
                                            break
                                        else:
                                            data['first_available_date'] = self.formatDate(
                                                i.xpath("text()")[0].strip(), "ca")
                                            break
                            except:
                                pass
                        else:
                            ...
                            if (first_available_date):
                                # print (first_available_date[0].strip())
                                data['first_available_date'] = self.formatDate(first_available_date[0].strip(), "ca")
                            else:
                    ...
                    if (sales_rank):
                        for i in range(len(sales_rank)):
                            ...
                            if (sales_rank[i][0].text.strip() == "Best Sellers Rank"):
                                ...
                                sales_rank = self.formatNumber(sales_rank[0].split("in")[0].strip(), "ca")
                        ...
                        if (sales_rank):
                            sales_rank = self.formatNumber(sales_rank[0].replace("\n", "").strip().split("in")[0].replace("See top 100", "").replace(".", "").strip(), "ca")
                            try:
                                if (sales_rank.isdigit()):
                                    data['sales_rank'] = sales_rank
                                    ...
                                    if (sales_rank):
                                        for i in sales_rank:
                                            if ("#" in i.replace("\n", "").strip()):
                                                # print (i.replace("\n", "").split("in")[0].replace("#", "").strip())
                                                data['sales_rank'] = i.replace("\n", "").split("in")[0].replace("#", "").strip()
                                                break
                                    else:
                                        print ("no sales_rank")
                            except:
                            ...
                            #                 if (i.xpath("strong/text()")[
                            #                         0].strip() == "Best Sellers Rank"):
                            #                     for j in i.xpath("text()"):
                            #                         if (j.replace("\n", "").strip() != ""):
                            #                             if ("in" in j.replace("\n", "").strip()):
                            #                                 if (j.replace("\n", "").strip().split("in")[0].replace("n.",
                            #                                                                                        "").replace(
                            #                                         ".", "").strip().isdigit()):
                ...
                if(category):
                    ...
                    if (category):
                        ...
                        if (category):
                            for i in category:
                                if (i.strip() != ""):
                                    if ("#" in i.strip() and "in" in i.strip()):
                                        # print (i.strip().split("in")[1].replace("(", "").strip())
                                        data['category'] = i.strip().split("in")[1].replace("(", "").strip()
                                        break
                        else:
                            print ("no category")
        except:
        ...
        try:
            ...
            if(browse_node):
                if (browse_node[0].replace("https://www.amazon.ca/gp/bestsellers/" , "").split("/")[0] != ''):
                    if (browse_node[0].replace("https://www.amazon.ca/gp/bestsellers/" , "").split("/")[0] != "https:"):
                        data['browse_node'] = browse_node[0].replace("https://www.amazon.ca/gp/bestsellers/" , "").split("/")[0]
                    else:
                ...
                if (browse_node):
                    ...
                    if (browse_node):
                        if (browse_node[0].replace("https://www.amazon.ca/gp/bestsellers/", "").split("/")[0] != ''):
                            if (browse_node[0].replace("https://www.amazon.ca/gp/bestsellers/", "").split("/")[
                                    0] != "https:"):
                                data['browse_node'] = \
                                browse_node[0].replace("https://www.amazon.ca/gp/bestsellers/", "").split("/")[0]
                            else:
                                if (browse_node[0].split("/")[-3]):
                                    data['browse_node'] = browse_node[0].split("/")[-3]
                                else:
                        ...
                        if (browse_node):
                            if (browse_node[0].replace("https://www.amazon.ca/gp/bestsellers/", "").split("/")[0] != ''):
                                if (browse_node[0].replace("https://www.amazon.ca/gp/bestsellers/", "").split("/")[
                                        0] != "https:"):
                                    data['browse_node'] = \
                                        browse_node[0].replace("https://www.amazon.ca/gp/bestsellers/", "").split("/")[
                                            0]
                                else:
                                    if (browse_node[0].split("/")[-3]):
                                        data['browse_node'] = browse_node[0].split("/")[-3]
        ...
        try:
            if(rating):
                if(rating[0].strip() != ''):
                    data['rating'] = self.formatRating(rating[0].strip(), "ca")
                    # print (data['rating'])
                else:
                    print("no rating")
            else:
        ...
        try:
            if(review_count):
                if (review_count[0].strip() != ''):
                    data['review_count'] = self.formatReviewCount(review_count[0].split(" ")[0].strip(), "ca")
