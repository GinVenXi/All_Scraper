...
class Model_Processor_Product_Base_Fr(Model_Processor):
    ...
    def process(self, html):
        ...
        try:
            ...
            if(price):
                if (price[0].strip() != ''):
                    data['price'] = self.formatNumber(price[0].strip(), "fr")
                    # print (data['price'])
            else:
        ...
        try:
            if(listprice):
                if (listprice[0].strip() != ''):
                    data['list_price'] = self.formatNumber(listprice[0].strip(), "fr")
        ...
        try:
            ...
            if(offer_count):
                if(offer_count[0].strip() != ''):
                    count = offer_count[0].strip().split(" ")[0].strip()
                    if (count.isdigit()):
        ...
        try:
            if(seller_link):
                for sel in seller_link:
                    if(sel.text.strip() == "Expédié et vendu par Amazon EU Sarl."):
                        data['seller_name'] = 'Amazon.fr'
                        data['fulfillment'] = 'AFN'
                        data['is_fba'] = '1'
                        print ("Amazon no seller_id")
                        break
                    elif (sel.text.strip() == "Expédié et vendu par Amazon."):
                        data['seller_name'] = 'Amazon.fr'
                        data['fulfillment'] = 'AFN'
                        data['is_fba'] = '1'
                        print ("Amazon no seller_id")
                        ...
                        if(seller_name):
                            ...
                            if (seller_name):
                                if (seller_name[0].strip() == "Expédié et vendu par Amazon"):
                                    data['seller_name'] = 'Amazon.fr'
                                    data['fulfillment'] = 'AFN'
                                    data['is_fba'] = '1'
                                    print ("Amazon no seller_id")
                            ...
                            if(fulfillment):
                                ...
                                if (fulfillment):
                                    for ful in fulfillment:
                                        if (ful.strip().split(".")[0] == "Expédié et vendu par Amazon EU Sarl."):
                                            data['seller_name'] = 'Amazon.fr'
                                            data['fulfillment'] = 'AFN'
                                            data['is_fba'] = '1'
                                            print ("Amazon no seller_id")
            ...
            if(first_available_date):
                ...
                for first_available in first_available_date:
                    for first in first_available:
                        try:
                            if(first.text != None):
                                if(first.text.strip() == "Im Angebot von Amazon.fr seit:"):
                                    first_cnt = 1
                                    continue
                                if(first_cnt == 1):
                                    data['first_available_date'] = self.formatDate(first.text.strip(), "fr")
                                    break
                            else:
                                break
                        except Exception as err:
                ...
                if (first_available_date):
                    for i in first_available_date:
                        if (i[0].text.strip() != ''):
                            if (i[0].text.strip() == "Date de mise en ligne sur Amazon.fr :"):
                                data['first_available_date'] = self.formatDate(i[1].text.strip(), "fr")
                                break
                else:
                    ...
                    if (first_available_date):
                        for i in first_available_date:
                            try:
                                ...
                                if (i.xpath("b/text()")[0].strip() == "Date de mise en ligne sur Amazon.fr :"):
                                    data['first_available_date'] = self.formatDate(i.xpath("text()")[0].strip(), "fr")
                                    break
                                elif (i.xpath("b/text()")[0].strip() == "Date de sortie d'origine:"):
                                    # print self.formatDate(i.xpath("text()")[0].strip(), "fr")
                                    data['first_available_date'] = self.formatDate(i.xpath("text()")[0].strip(), "fr")
                                    break
                                ...
                                try:
                                    # print (i.xpath("b/text()")[0].replace("\n", "").strip())
                                    if (i.xpath("strong/text()")[0].strip() == "Date de mise en ligne sur Amazon.fr :"):
                                        data['first_available_date'] = self.formatDate(i.xpath("text()")[0].strip(), "fr")
                                        break
                        ...
                        if (first_available_date):
                            try:
                                for i in first_available_date:
                                    ...
                                    if (i.xpath("b/text()")[0].strip() == "Date de mise en ligne sur Amazon.fr :"):
                                        data['first_available_date'] = self.formatDate(i.xpath("text()")[0].strip(), "fr")
                                        break
                                    elif (i.xpath("b/text()")[0].strip() == "Editeur :"):
                                        data['first_available_date'] = self.formatDate(i.xpath("text()")[0].strip().split("(")[1].replace(")", ""), "fr")
                                        break
                            except Exception as err:
                                print (err)
                                for i in first_available_date:
                                    try:
                                        ...
                                        if (i.xpath("b/text()")[0].strip() == "Publisher:"):
                                            data['first_available_date'] = self.formatDate(i.xpath("text()")[0].strip().split("(")[1].replace(")", ""), "fr")
                                            break
                                        if (i.xpath("b[1]/text()")[0].strip() == "Editeur :"):
                                            data['first_available_date'] = self.formatDate(i.xpath("text()")[1].strip().split("(")[1].replace(")", ""), "fr")
                                            break
                            ...
                            try:
                                if (first_available_date):
                                    data['first_available_date'] = self.formatDate(first_available_date[0], "fr")
        ...
        try:
            ...
            if(sales_rank):
                ...
                sales_rank = tree.xpath(".//*[@id='SalesRank']/ul/li[1]/span[1]")
                if(sales_rank):
                    if (sales_rank[0].text.replace("#" , "") != '' and self.formatNumber(sales_rank[0].text.strip(), "fr").isdigit()):
                        data['sales_rank'] = self.formatNumber(sales_rank[0].text.strip(), "fr")
                    else:
                        print("no sales_rank")
                else:
                    ...
                    if (sales_rank):
                        for i in range(len(sales_rank)):
                            ...
                            if (sales_rank[i][0].text.strip() == "Best Sellers Rank"):
                                ...
                                sales_rank = self.formatNumber(sales_rank[0].split("dans la")[0].replace(".", "").strip(), "fr")
                                    ...
                                    if (sales_rank):
                                        for i in sales_rank:
                                            if (i.replace("\n", "").replace("(", "").replace(")", "").strip()):
                                                ...
                                                # print (i.replace("\n", "").replace("(", "").replace(")", "").strip())
                                                if (self.formatNumber(sales_rank.split("en")[0].strip(), "fr").isdigit()):
                                                    data['sales_rank'] = self.formatNumber(sales_rank.split("en")[0].strip(), "fr")
                                                    break
                            except:
                                pass
                        else:
                                ...
                                for i in sales_rank:
                                    try:
                                        if (i.xpath("strong/text()")[0].strip() != ""):
                                            ...
                                            if (i.xpath("strong/text()")[0].strip() == "Classement des meilleures ventes d'Amazon:"):
                                                for j in i.xpath("text()"):
                                                    if (j.replace("\n", "").strip() != ""):
                                                        if ("dans la" in j.replace("\n", "").strip()):
                                                            if (j.replace("\n", "").strip().split("dans la")[0].replace(".", "").replace("n°", "").strip().isdigit()):
                                                                # print (j.replace("\n", "").strip().split("en")[0].strip())
                                                                data['sales_rank'] = self.formatNumber(j.replace("\n", "").strip().split("dans la")[0].replace(".", "").replace("n°", "").strip(), "fr")
                                                        elif ("en" in j.replace("\n", "").strip()):
                                                            if (j.replace("\n", "").strip().split("en")[0].replace(".", "").replace("n°", "").strip().isdigit()):
                                                                # print (j.replace("\n", "").strip().split("en")[0].strip())
                                                                data['sales_rank'] = self.formatNumber(j.replace("\n", "").strip().split("en")[0].replace(".", "").replace("n°", "").strip(), "fr")
                                    except:
                                        pass
                                    try:
                                        if (i.xpath("b/text()")[0].strip() != ""):
                                            # print (i.xpath("b/text()")[0].strip())
                                            if (i.xpath("b/text()")[0].strip() == "Classement des meilleures ventes d'Amazon:"):
                                                for j in i.xpath("text()"):
                                                    if (j.replace("\n", "").strip() != ""):
                                                        if ("dans la" in j.replace("\n", "").strip()):
                                                            if (j.replace("\n", "").strip().split("dans la")[0].replace(".", "").replace("n°", "").strip().isdigit()):
        ...
        try:
            ...
            if(category):
                if ("dans la" in category[0].strip()):
                    ...
                    data['category'] = category[0].replace("Voir les 100 premiers en ", "").strip()
            else:
        ...
        try:
            ...
            if(browse_node):
                if (browse_node[0].replace("https://www.amazon.fr/gp/bestsellers/" , "").split("/")[0] != ''):
                    if (browse_node[0].replace("https://www.amazon.fr/gp/bestsellers/" , "").split("/")[0] != "https:"):
                        data['browse_node'] = browse_node[0].replace("https://www.amazon.fr/gp/bestsellers/" , "").split("/")[0]
                    else:
        ...
        try:
            if(rating):
                if(rating[0].strip() != ''):
                    data['rating'] = self.formatRating(rating[0].strip(), "fr")
                    # print (data['rating'])
                else:
                    print("no rating")
            else:
        ...
        try:
            if(review_count):
                if (review_count[0].strip() != ''):
                    data['review_count'] = self.formatReviewCount(review_count[0].split(" ")[0].strip(), "fr")
