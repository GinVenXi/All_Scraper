...
class Model_Processor_Product_Base_It(Model_Processor):
    ...
    def process(self, html):
        ...
        try:
            ...
            if(price):
                if (price[0].strip() != ''):
                    data['price'] = self.formatNumber(price[0].strip(), "it")
                    # print (data['price'])
            else:
        ...
        try:
            if(listprice):
                if (listprice[0].strip() != ''):
                    data['list_price'] = self.formatNumber(listprice[0].strip(), "it")
        ...
        try:
            ...
            if(offer_count):
                    ...
                    # print (data['offer_count'])
                        ...
                        if (offer_count):
                            if (offer_count[0].strip() != ''):
                                ...
                                count = offer_count[0].strip().replace("(", "").replace(")", "").split(" ")[0].replace("nuovi", "").strip()
                                if (count.isdigit()):
                                    data['offer_count'] = count
                                    # print (data['offer_count'])
                                else:
                                    count = offer_count[0].strip().split(":")[1].strip()
                                    if (count.isdigit()):
                                        data['offer_count'] = count
                        else:
                ...
                if (offer_count):
                    ...
                    offer_count = tree.xpath("//*[@id='unqualifiedBuyBox']//text()")
                    if (offer_count):
                        for i in offer_count:
                            if ("Nuovi" in i):
                                # print (i.replace("\n", "").replace("Nuovi", "").replace(":", "").strip())
                                if (i.replace("\n", "").replace("Nuovi", "").replace(":", "").strip().isdigit()):
                                    count = i.replace("\n", "").replace("Nuovi", "").replace(":", "").strip()
                                    data['offer_count'] = count
                    else:
                        print ("no offer_count")
        ...
        try:
            if(seller_link):
                for sel in seller_link:
                    if("Venduto e spedito da Amazon" in sel.text.strip()):
                        data['seller_name'] = 'Amazon.it'
                        data['fulfillment'] = 'AFN'
                        data['is_fba'] = '1'
                        print ("Amazon no seller_id")
                        break
                    elif ("Venduto e spedito da Amazon EU Sarl" in sel.text.strip()):
                        data['seller_name'] = 'Amazon.it'
                        data['fulfillment'] = 'AFN'
                        data['is_fba'] = '1'
                        print ("Amazon no seller_id")
                        ...
                        seller_link = tree.xpath("//*[@id='pe-text-availability-merchant-info']/text()")
                        if (seller_link):
                            for sel in seller_link:
                                if ("Venduto e spedito da Amazon" in sel.strip()):
                                    data['seller_name'] = 'Amazon.it'
                                    data['fulfillment'] = 'AFN'
                                    data['is_fba'] = '1'
                                    print ("Amazon no seller_id")
                            ...
                            seller_name = tree.xpath("//*[@id='merchant-info']/b/a")
                            if(seller_name):
                                ...
                                if (seller_name):
                                    if (seller_name[0].text.strip() != ''):
                                        ...
                                        print("no seller_name")
                            fulfillment = tree.xpath("//*[@id='merchant-info']/a[1]")
                            # print (fulfillment)
                            try:
                                if(fulfillment):
                                    if("spedito da Amazon" in fulfillment[0].text.strip()):
            ...
            if(first_available_date):
                ...
                for first_available in first_available_date:
                    for first in first_available:
                        try:
                            if(first.text != None):
                                if(first.text.strip() == "Im Angebot von Amazon.it seit:"):
                                    first_cnt = 1
                                    continue
                                if(first_cnt == 1):
                                    data['first_available_date'] = self.formatDate(first.text.strip(), "it")
                                    break
                            else:
                                break
                        except Exception as err:
                ...
                if (first_available_date):
                    for i in first_available_date:
                        if (i[0].text.strip() != ''):
                            if (i[0].text.strip() == "Date de mise en ligne sur Amazon.it :"):
                                data['first_available_date'] = self.formatDate(i[1].text.strip(), "it")
                                break
                else:
                    ...
                    if (first_available_date):
                        for i in first_available_date:
                            try:
                                ...
                                if (i.xpath("b/text()")[0].strip() == "Date de mise en ligne sur Amazon.it :"):
                                    data['first_available_date'] = self.formatDate(i.xpath("text()")[0].strip(), "it")
                                    break
                                elif (i.xpath("b/text()")[0].strip() == "Date de sortie d'origine:"):
                                    # print self.formatDate(i.xpath("text()")[0].strip(), "it")
                                    data['first_available_date'] = self.formatDate(i.xpath("text()")[0].strip(), "it")
                                    break
                                    ...
                                    for j in i.xpath("text()"):
                                        if (j.replace("\n", "").strip() != ""):
                                            ...
                                            break
                                    break
                                elif (i.xpath("b/text()")[0].strip() == "Producto en Amazon.it desde:"):
                                    # print self.formatDate(i.xpath("text()")[0].strip(), "it")
                                    for j in i.xpath("text()"):
                                        if (j.replace("\n", "").strip() != ""):
                                            # print (j.replace("\n", "").strip())
                                            ...
                                            break
                                    break
                                elif (i.xpath("b/text()")[0].strip() == "Editore:"):
                                    # print self.formatDate(i.xpath("text()")[0].strip(), "it")
                                    for j in i.xpath("text()"):
                                        if (j.replace("\n", "").strip() != ""):
                                            # print (j.replace("\n", "").strip())
                                            data['first_available_date'] = self.formatDate(i.xpath("text()")[0].strip().split("(")[1].replace(")", ""), "it")
                                            break
                                    break
                            except Exception as err:
                                print (err)
                                try:
                                    # print (i.xpath("b/text()")[0].replace("\n", "").strip())
                                    if (i.xpath("strong/text()")[0].strip() == "Date de mise en ligne sur Amazon.it :"):
                                        data['first_available_date'] = self.formatDate(i.xpath("text()")[0].strip(), "it")
                                        break
                        ...
                        if (first_available_date):
                            try:
                                for i in first_available_date:
                                    ...
                                    if (i.xpath("b/text()")[0].strip() == "Date de mise en ligne sur Amazon.it :"):
                                        ...
                                        data['first_available_date'] = self.formatDate(date, "it")
                                        break
                                    elif (i.xpath("b/text()")[0].strip() == "Fecha de lanzamiento:"):
                                        data['first_available_date'] = self.formatDate(i.xpath("text()")[0].strip(), "it")
                                        break
                            ...
                            try:
                                if (first_available_date):
                                    data['first_available_date'] = self.formatDate(first_available_date[0], "it")
        ...
        try:
            ...
            if(sales_rank):
                ...
                if (" in " in sales_rank[1].replace("\n" , "").replace(" (" , "").strip()):
                    if (sales_rank[1].replace("\n", "").replace(" (", "").split("in")[0].replace("#", "").strip() != "" and self.formatNumber(sales_rank[1].replace("\n", "").replace(" (", "").split("in")[0].replace("n.", "").replace(".", "").strip(), "it").isdigit()):
                        # print (sales_rank[1].replace("\n" , "").replace(" (" , "").split("en")[0].strip())
                        data['sales_rank'] = self.formatNumber(sales_rank[1].replace("\n", "").replace(" (", "").split("in")[0].replace("n.", "").replace(".", "").strip(), "it")
                elif (sales_rank[1].replace("\n" , "").replace(" (" , "").strip() and self.formatNumber(sales_rank[1].replace("\n" , "").replace(" (" , "").strip().split(" ")[0].replace("#", ""), "it").isdigit()):
                    data['sales_rank'] = self.formatNumber(sales_rank[1].replace("\n" , "").replace(" (" , "").strip().split(" ")[0].replace("#", ""), "it")
                    # print (sales_rank[1].replace("\n" , "").replace(" (" , "").strip().split(" ")[0].replace("#", ""))
                else:
                    # cloth 分类下
                    sales_rank = tree.xpath("//*[@id='SalesRank']/ul/li[1]/span[1]")
                    if (sales_rank and self.formatNumber(sales_rank[0].text.strip(), "it").isdigit()):
                        data['sales_rank'] = self.formatNumber(sales_rank[0].text.strip(), "it")
                    else:
                        print("no sales_rank")
            else:
                # cloth 分类下
                sales_rank = tree.xpath(".//*[@id='SalesRank']/ul/li[1]/span[1]")
                if(sales_rank):
                    if (sales_rank[0].text.replace("#" , "") != '' and self.formatNumber(sales_rank[0].text.strip(), "it").isdigit()):
                        data['sales_rank'] = self.formatNumber(sales_rank[0].text.strip(), "it")
                    else:
                        print("no sales_rank")
                else:
                    ...
                    if (sales_rank):
                        for i in range(len(sales_rank)):
                            ...
                            if (sales_rank[i][0].text.strip() == "Best Sellers Rank"):
                                ...
                                sales_rank = self.formatNumber(sales_rank[0].split("dans la")[0].replace(".", "").strip(), "it")
                                        ...
                                        for i in sales_rank:
                                            if (i.replace("\n", "").strip() != ""):
                                                if (i.replace("\n", "").replace("(", "").replace(")", "").strip()):
                                                    ...
                                                    # print (i.replace("\n", "").replace("(", "").replace(")", "").strip())
                                                    if (self.formatNumber(sales_rank.split("en")[0].strip(), "it").isdigit()):
                                                        data['sales_rank'] = self.formatNumber(sales_rank.split("en")[0].strip(), "it")
                                                        break
                                        sales_rank = tree.xpath("//*[@id='prodDetails']//tr[@id='SalesRank']/td[@class='value']//text()")
                                        if (sales_rank):
                                            for i in sales_rank:
                                        ...
                                        if (i.xpath("strong/text()")[0].strip() != ""):
                                            ...
                                            if (i.xpath("strong/text()")[0].strip() == "Clasificación en los más vendidos de Amazon"):
                                                for j in i.xpath("text()"):
                                                    if (j.replace("\n", "").strip() != ""):
                                                        if ("in" in j.replace("\n", "").strip()):
                                                            if (j.replace("\n", "").strip().split("in")[0].replace("n.", "").replace(".", "").strip().isdigit()):
                                                                # print (j.replace("\n", "").strip().split("en")[0].strip())
                                                                data['sales_rank'] = j.replace("\n", "").strip().split("in")[0].replace("n.", "").replace(".", "").strip()
                                    except:
                                        pass
                                    try:
                                        if (i.xpath("b/text()")[0].strip() != ""):
        ...
        try:
            ...
            if(category):
                if ("in" in category[0].strip()):
                    data['category'] = category[0].replace("Visualizza i Top 100 nella categoria", "").replace("Visualizza i Top 100", "").strip()
                else:
                    category = tree.xpath("//*[@id='SalesRank']/text()")
                    if (category):
                            for i in category:
                                if ("in" in i.replace("\n", "").strip()):
                                    # print (i.replace("\n", "").strip().split("in")[1].replace("(", "").strip())
                                    data['category'] = i.replace("\n", "").strip().split("in")[1].replace("(", "").strip()
        ...
        try:
            ...
            if(browse_node):
                if (browse_node[0].replace("https://www.amazon.it/gp/bestsellers/" , "").split("/")[0] != ''):
                    if (browse_node[0].replace("https://www.amazon.it/gp/bestsellers/" , "").split("/")[0] != "https:"):
                        data['browse_node'] = browse_node[0].replace("https://www.amazon.it/gp/bestsellers/" , "").split("/")[0]
                    else:
        ...
        try:
            if(rating):
                if(rating[0].strip() != ''):
                    data['rating'] = self.formatRating(rating[0].strip(), "it")
                    # print (data['rating'])
                else:
                    print("no rating")
            else:
        ...
        try:
            if(review_count):
                if (review_count[0].strip() != ''):
                    data['review_count'] = self.formatReviewCount(review_count[0].split(" ")[0].strip(), "it")
