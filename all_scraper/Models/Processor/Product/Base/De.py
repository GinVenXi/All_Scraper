...
class Model_Processor_Product_Base_De(Model_Processor):

    def process(self, html):

        if html == '' or html == 'None':
            print "Can't get them html from https://www.amazon.de"
            sys.exit()
        ...
        try:
            if(listprice):
                if (listprice[0].strip() != ''):
                    data['list_price'] = self.formatNumber(listprice[0].strip(), "de")
        ...
        try:
            if(seller_link):
                for sel in seller_link:
                    if(sel.text.strip() == "Verkauf und Versand durch Amazon."):
                        data['seller_name'] = 'Amazon.de'
                        data['fulfillment'] = 'AFN'
                        data['is_fba'] = '1'
                        print ("Amazon no seller_id")
                    elif (sel.text.strip() == "Verkauf und Versand durch Amazon EU Sarl."):
                        data['seller_name'] = 'Amazon.de'
                        data['fulfillment'] = 'AFN'
                        data['is_fba'] = '1'
                        print ("Amazon no seller_id")
                            ...
                            if(fulfillment):
                                ...
                                if (fulfillment):
                                    for ful in fulfillment:
                                        if (ful.strip().split(".")[0] == "Verkauf und Versand durch Amazon."):
                                            data['seller_name'] = 'Amazon.de'
                                            data['fulfillment'] = 'AFN'
                                            data['is_fba'] = '1'
                                            print ("Amazon no seller_id")
            ...
            if(first_available_date):
                ...
                for first_available in first_available_date:
                    for first in first_available:
                        try:
                            if(first.text != None):
                                if(first.text.strip() == "Im Angebot von Amazon.de seit:"):
                                    first_cnt = 1
                                    continue
                                if(first_cnt == 1):
                                    data['first_available_date'] = self.formatDate(first.text.strip(), "de")
                                    break
                            else:
                                break
                        except Exception as err:
                ...
                if (first_available_date):
                    for i in first_available_date:
                        if (i[0].text.strip() != ''):
                            if (i[0].text.strip() == "Im Angebot von Amazon.de seit:"):
                                data['first_available_date'] = self.formatDate(i[1].text.strip(), "de")
                                break
                            elif (i[0].text.strip() == "Verlag:"):
                                data['first_available_date'] = self.formatDate(i[1].text.strip(), "de")
                                break
                else:
                    ...
                    if (first_available_date):
                        for i in first_available_date:
                            try:
                                ...
                                if (i.xpath("b/text()")[0].strip() == "Im Angebot von Amazon.de seit:"):
                                    data['first_available_date'] = self.formatDate(i.xpath("text()")[0].strip(), "de")
                                    break
                                elif (i.xpath("b/text()")[0].strip() == "Verlag:"):
                                    # print self.formatDate(i.xpath("text()")[0].strip(), "de")
                                    data['first_available_date'] = self.formatDate(i.xpath("text()")[0].strip(), "de")
                                    break
                                elif (i.xpath("b/text()")[0].strip() == "Audio CD"):
                                    # print self.formatDate(i.xpath("text()")[0].strip(), "de")
                                    data['first_available_date'] = self.formatDate(i.xpath("text()")[0].strip(), "de")
                                    break
                            except:
                        ...
                        if (first_available_date):
                                ...
                                for i in first_available_date:
                                    try:
                                        if (i.xpath("b/text()")[0].strip() == "Im Angebot von Amazon.de seit:"):
                                            ...
                                            data['first_available_date'] = self.formatDate(i.xpath("text()")[0].strip().split("(")[1].replace(")", ""), "de")
                                            break
                                        elif (i.xpath("b/text()")[0].strip() == "Publisher:"):
                                            data['first_available_date'] = self.formatDate(i.xpath("text()")[0].strip().split("(")[1].replace(")", ""), "de")
                                            break
                            ...
                            try:
                                if (first_available_date):
                                    data['first_available_date'] = self.formatDate(first_available_date[0], "de")
        ...
        try:
            ...
            if(sales_rank):
                if(sales_rank[1].replace("\n" , "").replace(" (" , "").split("in")[0].replace("#" , "").strip() != "" and self.formatNumber(sales_rank[1].replace("\n" , "").replace(" (" , "").split("in")[0].strip(), "de").isdigit()):
                    data['sales_rank'] = self.formatNumber(sales_rank[1].replace("\n" , "").replace(" (" , "").split("in")[0].strip(), "de")
                elif (sales_rank[1].replace("\n" , "").strip().split(" ")[0].replace("#" , "").strip() != "" and self.formatNumber(sales_rank[1].replace("\n" , "").strip().split(" ")[0].replace("#" , "").strip(), "de").isdigit()):
                    data['sales_rank'] = self.formatNumber(sales_rank[1].replace("\n" , "").strip().split(" ")[0].replace("#" , "").strip(), "de")
                else:
                    # cloth 分类下
                    sales_rank = tree.xpath(".//*[@id='SalesRank']/ul/li[1]/span[1]")
                    if (sales_rank):
                        if (self.formatNumber(sales_rank[0].text.strip(), "de").isdigit()):
                            data['sales_rank'] = self.formatNumber(sales_rank[0].text.strip(), "de")
                        else:
                            print ("no sales_rank")
                    else:
                        print("no sales_rank")
                    ...
                    if (sales_rank):
                        for i in range(len(sales_rank)):
                            ...
                            if (sales_rank[i][0].text.strip() == "Best Sellers Rank"):
                                ...
                                sales_rank = self.formatNumber(sales_rank[0].split("in")[0].strip(), "de")
        ...
        try:
            ...
            if(category):
                data['category']  = category[0].replace("Siehe Top 100 in " , "").strip()
            else:
        ...
        try:
            ...
            if(browse_node):
                if (browse_node[0].replace("https://www.amazon.de/gp/bestsellers/" , "").split("/")[0] != ''):
                    if (browse_node[0].replace("https://www.amazon.de/gp/bestsellers/" , "").split("/")[0] != "https:"):
                        data['browse_node'] = browse_node[0].replace("https://www.amazon.de/gp/bestsellers/" , "").split("/")[0]
                    else:
        ...
        try:
            if(rating):
                if(rating[0].strip() != ''):
                    data['rating'] = self.formatRating(rating[0].strip(), "de")
                    # print (data['rating'])
                else:
                    print("no rating")
            else:
                ...
                if (rating):
                    ...
                    if (rating):
                        ...
                        rating = tree.xpath(
                            "//*[contains(@class, 'a-spacing-top-micro')]//i[contains(@class, 'a-icon-star')]/span/text()")
                        if (rating):
                            if (rating[0].strip() != ''):
                                data['rating'] = self.formatRating(rating[0].strip(), "de")
                        else:
        ...
        try:
            if(review_count):
                if (review_count[0].strip() != ''):
                    data['review_count'] = self.formatReviewCount(review_count[0].split(" ")[0].strip(), "de")
                ...
                if (review_count):
                    ...
                    if (review_count):
                        ...
                        review_count = tree.xpath(
                            "//*[contains(@class, 'a-spacing-top-micro')]//span[@class='a-size-small']/a/text()")
                        if (review_count):
                            if (review_count[0].strip() != ''):
                                data['review_count'] = self.formatReviewCount(
                                    review_count[0].replace("\n", "").strip().split(" ")[0].strip(), "de")
                                # print (data['review_count'])
                        else:
                            print("no review_count")
        except:
