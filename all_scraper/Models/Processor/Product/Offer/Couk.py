...
class Model_Processor_Product_Offer_Couk(Model_Processor):
    def __init__(self):
        pass

    def process(self, html):
        if html == '' or html == 'None':
            print "Can't get them html from https://www.amazon.co.uk/offerlist"
            sys.exit()
        ...
        for itemDom in div:
            ...
            try:
                ...
                if (condition):
                    if (condition[0].replace("\n", "").strip() != ''):
                        data['condition'] = condition[0].replace("\n", "").replace(" ", "").replace("-", "").strip()
            ...
            try:
                ...
                if (fulfillment):                       # Fulfilled by Amazon
                    if (fulfillment[0].strip() == 'Fulfillment by Amazon' or fulfillment[0].strip() == 'Fulfilled by Amazon'):
            ...
            try:
                ...
                if (seller_rating):
                    data['seller_rating'] = self.formatRating(seller_rating[0].strip(), "co.uk")
            ...
            try:
                ...
                if (seller_rating_count):
                    try:
                        rating_count = self.formatSellerRatingCount(seller_rating_count[2].strip().split(".")[1], "co.uk").strip()
                    except:
                        pass
            ...
            try:
                ...
                if (seller_positive):
                    if (seller_positive[0].strip()):
                        if (self.formatPositive(seller_positive[0].replace(".", "").strip(), "co.uk").isdigit()):
                            data['seller_positive'] = self.formatPositive(seller_positive[0].replace(".", "").strip(), "co.uk")
