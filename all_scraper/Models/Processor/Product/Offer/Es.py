...
class Model_Processor_Product_Offer_Es(Model_Processor):
    def __init__(self):
        pass

    def process(self, html):
        if html == '' or html == 'None':
            print "Can't get them html from https://www.amazon.es/offerlist"
            sys.exit()
        ...
        for itemDom in div:
            ...
            try:
                ...
                if (fulfillment):                   # Gestionado por Amazon
                    if (fulfillment[0].strip() == 'Gestionado por Amazon'):
            ...
            try:
                ...
                if (seller_rating):
                    data['seller_rating'] = self.formatRating(seller_rating[0].strip(), "es")
            ...
            try:
                ...
                if (seller_rating_count):
                    try:
                        ...
                        rating_count = self.formatSellerRatingCount(seller_rating_count[2].strip().split("(")[1].replace(")", "").strip(), "es")
                    except:
                        pass
            ...
            try:
                ...
                if (seller_positive):
                    if (seller_positive[0].strip() != ""):
                        if (self.formatPositive(seller_positive[0].replace(".", "").strip(), "es").isdigit()):
                            data['seller_positive'] = self.formatPositive(seller_positive[0].replace(".", "").strip(), "es")
