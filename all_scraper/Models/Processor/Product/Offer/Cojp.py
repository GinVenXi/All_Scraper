...
class Model_Processor_Product_Offer_Cojp(Model_Processor):
    def __init__(self):
        pass

    def process(self, html):
        if html == '' or html == 'None':
            print "Can't get them html from https://www.amazon.co.jp/offerlist"
            sys.exit()
        ...
        for itemDom in div:
            ...
            # 包邮方式 shipping
            try:
                shipping1 = itemDom.xpath("div//p[contains(@class, 'olpShippingInfo')]/span/text()")
                shipping2 = itemDom.xpath("div//p[contains(@class, 'olpShippingInfo')]/span/b/text()")
                try:
                    ship1 = ""
                    if (shipping1):
                        ship1 = shipping1[0].replace("\n", "").strip()
                        if (ship1 == "+"):
                            shipping1 = itemDom.xpath("div//span[@class='olpShippingPrice']/text()")
                            if (shipping1):
                ...
                try:
                    ship2 = ""
                    if (shipping2 and shipping2 != ''):
                        ship2 = shipping2[0].replace("\n", "").strip()
                    else:
                        shipping2 = itemDom.xpath("div//span[@class='olpShippingPriceText']/text()")
                        if(shipping2):
            ...
            try:
                ...
                if (fulfillment):                   # Amazon.co.jp 配送センターより発送されます
                    if (fulfillment[0].strip() == 'Amazon.co.jp 配送センターより発送されます'):
            ...
            try:
                ...
                if (seller_rating):
                    data['seller_rating'] = self.formatRating(seller_rating[0].strip(), "co.jp")
            ...
            try:
                ...
                if (seller_rating_count):
                    try:
                        ...
                        rating_count = self.formatSellerRatingCount(seller_rating_count[2].strip().split("(")[1].replace(")", "").strip(), "co.jp")
                    except:
                        pass
            ...
            try:
                ...
                if (seller_positive):
                    if (seller_positive[0].strip() != ""):
                        if (self.formatPositive(seller_positive[0].replace(".", "").strip(), "co.jp").isdigit()):
                            data['seller_positive'] = self.formatPositive(seller_positive[0].replace(".", "").strip(), "co.jp")
